...
class ImprovedLineOfCreditApprovalModel:
    ...
    def train_approval_model(self):
        ...
        pipeline = Pipeline([
            ('imputer', SimpleImputer(strategy='median')),
            ('classifier', RandomForestClassifier(n_estimators=300, class_weight='balanced', max_depth=10, min_samples_split=5, random_state=42))
        ])
    ...
    def get_denial_reasons(self, applicant_df):
        reasons = []
        dti = applicant_df['dti'].values[0]
        income = applicant_df['annual_income'].values[0]
        credit_utilization = applicant_df['credit_utilization'].values[0]
        months_employed = applicant_df['months_employed'].values[0]
        payment_history = applicant_df['payment_history'].values[0]
        ...
        if payment_history != self.le_dict['payment_history'].transform(['On Time'])[0]:
            reasons.append("Poor payment history")

        return reasons if reasons else ["Not eligible due to internal model decision"]

    def apply_credit_score_cap(self, credit_score, predicted_amount):
        if credit_score >= 750:
            cap = 25000
        elif 660 <= credit_score < 750:
            cap = 15000
        elif 500 <= credit_score < 660:
            cap = 10000
        else:
            cap = 5000
        return min(predicted_amount, cap)
    ...
    def predict_loan_eligibility(self, applicant_data):
        ...
        if is_approved:
            ...
            raw_approved_amount = np.expm1(log_amount)
            # Apply strict credit score cap
            approved_amount = self.apply_credit_score_cap(predicted_credit_score, raw_approved_amount)
            interest_rate = self.interest_rate_model.predict(X_applicant)[0]
            denial_reasons = []
        else:
            approved_amount = 0
            interest_rate = None
            denial_reasons = self.get_denial_reasons(applicant_df)
